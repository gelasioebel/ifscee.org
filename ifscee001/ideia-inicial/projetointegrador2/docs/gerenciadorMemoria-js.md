# Implementação Completa do GerenciadorMemoria

Aqui está a implementação completa da classe `GerenciadorMemoria`, que simula o gerenciamento de memória em C para nosso sistema IFSCEE:
...
## Características Principais do GerenciadorMemoria

1. **Simulação Completa do Modelo de Memória C**
    - Gerencia pilha (stack frames com variáveis locais)
    - Gerencia heap (alocação dinâmica)
    - Gerencia variáveis globais e estáticas
    - Segmento de dados para strings literais

2. **Simulação Fiel das Funções de Memória**
    - `malloc`/`calloc` (alocaMemoria)
    - `free` (liberaMemoria)
    - `realloc` (realocaMemoria)
    - Acesso e modificação de memória (leMemoria, escreveMemoria)

3. **Gerenciamento de Tipos e Alinhamento**
    - Suporte a tipos primitivos (int, char, float, etc.)
    - Alinhamento de memória conforme especificações C
    - Suporte a arrays de tipos primitivos
    - Suporte a ponteiros
...
4. **Detecção de Problemas**
    ...
    - Operações em memória não alocada

5. **Stack Frames e Escopo**
    - Pilha de frames de execução
    - Gerenciamento de escopo de variáveis
    - Passagem de parâmetros para funções
    - Valores de retorno de funções

6. **Monitoramento e Diagnóstico**
    - Histórico de operações na memória
    - Estado detalhado da memória por região
    - Estatísticas de uso de memória
    - Registros de alterações em variáveis

7. **Segurança e Validação**
    - Verificações de limites em todas as operações
    - Validação de acesso à memória
    - Detecção de stack overflow (limite de recursão)
    - Proteção do segmento de dados constantes

Esta implementação do `GerenciadorMemoria` fornece uma simulação completa e fiel do modelo de memória da linguagem C, permitindo a visualização e depuração passo a passo da execução de programas C no ambiente IFSCEE.